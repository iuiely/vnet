...
class Dhcp_lib{
    ...
    public function add_dhcp_server($namespace,$ifname,$tag,$dhcprange,$netmask,$gateway,$dhcphostfile,$pidfile){
        ...
        if(!empty($namespace) && !empty($ifname) && !empty($tag) && !empty($dhcprange) && !empty($netmask) && !empty($gateway) && !empty($dhcphostfile)){
            ...
            $command = 'ip netns exec '.$namespace.' dnsmasq --no-hosts --no-resolv --strict-order --no-ping --bind-interfaces --interface='.$ifname.' --except-interface=lo --pid-file='.$pidfile.' --dhcp-range=set:'.$tag.','.explode(',',$dhcprange)[0].',static,'.$netmask.',2h --dhcp-option=option:router,'.$gateway.' --dhcp-hostsdir='.$dhcphostfile.' 2>&1';
